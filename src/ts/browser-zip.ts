...
(() => {
  ...
  function toDosDateTime(year: number, month: number, day: number, hour: number, minute: number, second: number): {
    dosDate: number;
    dosTime: number;
  } {
    return {
  ...
  function createLocalFileHeader(entry: StoredZipPreparedEntry): Uint8Array {
  ...
  function createCentralDirectoryHeader(entry: StoredZipPreparedEntry): Uint8Array {
  ...
  function createStoredZip(entries: Array<{ name: string; data: Uint8Array }>): Uint8Array {
    ...
    for (const entry of entries) {
      const preparedEntry: StoredZipPreparedEntry = {
